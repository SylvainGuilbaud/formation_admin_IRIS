...
class FhirConverterSimpleProcess(BusinessProcess):

    def on_fhir_converter_message(self, request: FhirConverterMessage):
        resource = None
        if request.root_template.find("ADT") != -1:
            resource = self.parse_adt_message(request.input_data.replace("\n", "\r"))

        return FhirConverterResponse(
            status=200,
            output_data=json.dumps(resource, ensure_ascii=False),
            output_filename=''
        )


    def parse_adt_message(self,hl7msg: str) -> dict:
        m = parse_message(hl7msg)
        ...
        if pid_5 := m.PID.PID_5:
            ...
            for pid_5_rep in pid_5:
                r["name"].append(self._parse_name_field(pid_5_rep))
        ...
        if pid_11 := m.PID.PID_11:
            ...
            for pid_11_rep in pid_11:
                r["address"].append(self._parse_address_field(pid_11_rep))
        ...
        if pid_13 or pid_14:
            r["telecom"] = self._parse_telecom_fields(pid_13, pid_14)
        ...
        if m.NK1:
            ...
            for nk1 in m.NK1:
                contact = dict()
                if nk1_2 := nk1.NK1_2:
                    contact["name"] = self._parse_name_field(nk1_2)
                if nk1_4 := nk1.NK1_4:
                    contact["address"] = self._parse_address_field(nk1_4)
                nk1_5, nk1_6 = nk1.NK1_5, nk1.NK1_6
                if nk1_5 or nk1_6:
                    contact["telecom"] = self._parse_telecom_fields(nk1_5, nk1_6)
    ...
    def _parse_telecom_fields(
        self,personal_telecom_field: Field, work_telecom_field: Field
    ) -> list:
        telecom_list = list()
        for field in [personal_telecom_field, work_telecom_field]:
            if field:
                for rep in field:
                    t = dict()
                    if use_value := self._get_telecom_use_value(rep.XTN_2.value):
                        t["use"] = use_value
    ...
    def _get_telecom_use_value(self,use_code: str) -> str:
        mapping = dict(
            PRN="home",
            TMP="temp",
            OLD="old",
    ...
    def _parse_name_field(self,name_field: Field) -> list:
        name_entry = dict()
        ...
        if use := self._get_name_type_value(name_field.XPN_7.value):
            name_entry["use"] = use
        return name_entry


    def _parse_address_field(self,address_field: Field) -> list:
    ...
    def _get_name_type_value(self,value: str) -> str:
        mapping = dict(
            U="usual",
            L="official",
            N="nickname",
